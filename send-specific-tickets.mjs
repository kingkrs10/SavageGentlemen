...
const requests = [
  {
    ...
    price: 10.0,
    qrPrefix: 'EVENT-2-MANUAL-SHANICES814',
    count: 1
  },
  {
    ...
    price: 0.0,
    qrPrefix: 'EVENT-2-MANUAL-KARISHMAA',
    count: 5
  },
  {
    ...
    price: 15.0,
    qrPrefix: 'EVENT-2-MANUAL-NATASHAPETERS',
    count: 1
  }
];

async function main() {
  console.log('=== SENDING EMAILS FOR MANUAL TICKETS ===\n');
  ...
  try {
    for (const req of requests) {
      ...
      for (let i = 0; i < req.count; i++) {
        const qrData = req.count === 1 ? req.qrPrefix : `${req.qrPrefix}-${i + 1}`;
