...
import { Search, User, LogOut, LayoutDashboard, Ticket, Bot } from "lucide-react";
...
const Header = ({ user: propUser, onProfileClick, onLogout }: HeaderProps) => {
  const { user: contextUser, logout, login } = useUser();
  ...
  useEffect(() => {
    const handleOpenAuthModal = (e: CustomEvent) => {
      const { tab, redirectPath } = e.detail || {};
      if (tab) {
        localStorage.setItem('authModalSelectedTab', tab);
      }
      if (redirectPath) {
    ...
    window.addEventListener('sg:open-auth-modal' as any, handleOpenAuthModal as any);

    // Check URL params for direct login/register links
    ...
    if (action === 'login' || action === 'register') {
      ...
      if (redirect) {
        localStorage.setItem('sg:auth:redirect', redirect);
      }
    ...
    return () => {
      window.removeEventListener('sg:open-auth-modal' as any, handleOpenAuthModal as any);
    };
  }, []);
        ...
        <div className="container mx-auto px-4 flex flex-col">
          ...
          <div className="py-3 flex justify-between items-center border-t dark:border-white/10 light:border-black/10">
            <div className="flex items-center">
              <Image src={LogoImage} alt="Savage Gentlemen Logo" className="h-12 w-auto" />
