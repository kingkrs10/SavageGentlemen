...
const EventReviews = ({ eventId, eventTitle }: EventReviewsProps) => {
  ...
  const renderStars = (rating: number, interactive = false, size = "w-5 h-5") => {
    return [...Array(5)].map((_, i) => (
      <Star
        key={i}
        className={`${size} cursor-${interactive ? 'pointer' : 'default'} ${i < (interactive ? (hoveredRating || newReview.rating) : rating)
        ...
        {reviews.length > 0 ? (
          <div className="space-y-4">
            {displayedReviews.map((review: Review) => (
              <div key={review.id} className={`border rounded-lg p-4 ${currentUser && review.user.id === currentUser.id ? 'bg-blue-50 border-blue-200' : ''
                }`}>
