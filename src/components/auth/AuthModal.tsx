...
const AuthModal = ({ isOpen, onClose, onAuthSuccess, onGuestLogin }: AuthModalProps) => {
  ...
  // Check for stored tab preference in localStorage safely
  const [currentTab, setCurrentTab] = useState<"login" | "register">(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('authModalSelectedTab');
      ...
      <DialogContent className="bg-gray-900 sm:max-w-md">
        <DialogHeader className="text-center">
          <div className="flex justify-center mb-4">
            <Image src={LogoImg} alt="Savage Gentlemen Logo" className="h-32 w-auto object-contain" />
          </div>
