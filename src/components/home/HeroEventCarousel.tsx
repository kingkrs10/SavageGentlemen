          ...
          {events.map((event, index) => {
            ...
            return (
              <CarouselItem key={event.id} className="pl-0 h-full relative">
                ...
                <div className="absolute inset-0 overflow-hidden bg-gradient-to-br from-gray-900 via-orange-950/20 to-black">
                  {event.imageUrl && (
                    <motion.img
                      src={event.imageUrl}
                      alt={event.title}
                  ...
                  {/* Enhanced Cinematic Gradients - Significantly Reduced Opacity for maximum flyer visibility */}
                  <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent opacity-30" />
                  <div className="absolute inset-0 bg-gradient-to-r from-black/40 via-transparent to-black/40 opacity-20 md:opacity-30" />
                ...
                {isActive && (
                  <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-6 z-10 top-0">
                    ...
                    >
                      {event.category && (
                        <Badge className="bg-primary/20 text-primary border border-primary/50 backdrop-blur-md text-sm px-4 py-1 uppercase tracking-[0.2em] mb-2 shadow-[0_0_15px_rgba(255,87,34,0.3)]">
                          {event.category}
                        </Badge>
                      )}

                      <h1 className="text-5xl md:text-7xl lg:text-[8rem] font-heading text-white uppercase tracking-tighter leading-[0.85] drop-shadow-2xl mix-blend-plus-lighter opacity-90">
                        {event.title}
                      </h1>

                      <p className="text-lg md:text-xl text-gray-200 font-light tracking-wide max-w-2xl mx-auto drop-shadow-lg leading-relaxed bg-black/10 backdrop-blur-sm rounded-lg px-4 py-2">
