...
const ProductCard = ({
  ...
  const getImageUrl = (url: string | null | undefined) => {
    if (!url || url.trim() === '') {
      return SGFlyerLogoPng.src; // Fallback to brand logo for empty URLs
    }
  ...
  if (variant === "large") {
    return (
      <div className="bg-card rounded-3xl overflow-hidden shadow-lg group cursor-pointer border border-white/5">
        <a
          href={product.etsyUrl || undefined}
          target="_blank"
          rel="noopener noreferrer"
          className="block relative group-hover:scale-105 transition-transform duration-300"
        >
          <img
            src={imgError ? SGFlyerLogoPng.src : imgSrc}
            alt={title}
            className="w-full h-48 object-contain p-6 bg-card"
            loading="lazy"
  ...
  return (
    <div className="modern-card glass-card animate-fade-in-up group cursor-pointer border border-white/5 bg-white/5 hover:bg-white/10 transition-colors">
      <a
        href={product.etsyUrl || undefined}
        target="_blank"
        rel="noopener noreferrer"
      ...
      >
        <div className="w-full h-40 bg-gradient-to-br from-black via-card to-black flex items-center justify-center p-4">
          <img
            src={imgError ? SGFlyerLogoPng.src : imgSrc}
            alt={title}
