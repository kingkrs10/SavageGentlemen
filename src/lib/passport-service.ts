...
export class PassportService {
    ...
    async getOrCreateProfile(userId: number, handle?: string): Promise<PassportProfile> {
        ...
        if (!profile) {
            ...
            await passportApi.createPassportMembership({
                userId,
                tierId: 1, // Default to lowest tier initially
                active: true,
            });
        }
    ...
    async awardStamp(
        ...
        const { stamp, checkin, creditTransaction } = await db.transaction(async (tx) => {
            ...
            const [creditTransaction] = await tx.insert(passportCreditTransactions).values({
                ...
                sourceId: stamp.id,
                balanceAfter: (profile.totalPoints || 0) + creditsToAward
            }).returning();
        ...
        const tierUpdate = await this.checkAndUpdateTier(userId, profile.totalPoints || 0, previousTier || 'BRONZE');
    ...
    private async checkAndUpdateTier(
        ...
        for (const tier of sortedTiers) {
            if (totalPoints >= (tier.minPoints || 0)) {
                qualifiedTier = tier.name || 'BRONZE';
            }
        }
    ...
    private async checkAndUnlockAchievements(userId: number): Promise<any[]> {
        ...
        for (const achievement of allAchievements) {
            ...
            if (criteria.eventsAttended && (profile.totalEvents || 0) >= criteria.eventsAttended) shouldUnlock = true;
            if (criteria.countriesVisited && (profile.totalCountries || 0) >= criteria.countriesVisited) shouldUnlock = true;
            if (criteria.creditsEarned && (profile.totalPoints || 0) >= criteria.creditsEarned) shouldUnlock = true;
    ...
    private async checkAndUnlockRewards(
        ...
        for (const milestone of milestones) {
            if ((profile.totalEvents || 0) >= milestone.stamps) {
    ...
    async getTierProgress(userId: number): Promise<TierProgressInfo> {
        ...
        const nextTier = tiers.find(t => (t.minPoints || 0) > (profile.totalPoints || 0));
        let pointsToNextTier = 0;
        let progressPercentage = 100;

        if (nextTier) {
            pointsToNextTier = (nextTier.minPoints || 0) - (profile.totalPoints || 0);
            const range = (nextTier.minPoints || 0) - (currentTier.minPoints || 0);
            const pointsInRange = (profile.totalPoints || 0) - (currentTier.minPoints || 0);
