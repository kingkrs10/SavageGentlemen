...
const DEFAULT_FROM_EMAIL = process.env.GMAIL_EMAIL || 'info@savgent.com';
...
export const sendEmail = async (options: EmailOptions): Promise<boolean> => {
  try {
    ...
    console.log(`[GMAIL] Attempting to send email to: ${to}, subject: ${subject}`);
    console.log(`[GMAIL] From: ${from}`);
    console.log(`[GMAIL] SMTP configured: ${process.env.GMAIL_EMAIL ? 'Yes' : 'No'}`);
...
/**
 * Send a ticket confirmation email with QR code using Gmail
 */
...
Questions? Contact us at: info@savgent.com
    ...
    if (result) {
      console.log(`[GMAIL] Ticket email sent successfully to ${recipientEmail}`);
    } else {
      console.error(`[GMAIL] Failed to send ticket email to ${recipientEmail}`);
    }
    
    return result;
    
  } catch (error) {
    console.error('[GMAIL] Error in sendTicketEmail:', error);
    return false;
  }
};
...
export const testEmailConnection = async (): Promise<boolean> => {
  try {
    console.log('[GMAIL] Testing email connection...');
    await transporter.verify();
