import { pgTable, text, serial, integer, boolean, timestamp, jsonb, uuid, date, numeric, uniqueIndex, index } from "drizzle-orm/pg-core";
...
export const insertPassportTierSchema = createInsertSchema(passportTiers)
  ...
  .extend({
    ...
    perks: z.record(z.string(), z.any()).default({}),
  });
...
export const passportCreditTransactions = pgTable("passport_credit_transactions", {
  ...
  return {
    userIdIdx: index("passport_credit_transactions_user_id_idx").on(table.userId),
    createdAtIdx: index("passport_credit_transactions_created_at_idx").on(table.createdAt),
  };
});
...
export const passportAchievementDefinitions = pgTable("passport_achievement_definitions", {
  ...
  return {
    ...
    categoryIdx: index("passport_achievement_definitions_category_idx").on(table.category),
  };
});
...
export const passportRedemptionOffers = pgTable("passport_redemption_offers", {
  ...
  return {
    ...
    categoryIdx: index("passport_redemption_offers_category_idx").on(table.category),
    isActiveIdx: index("passport_redemption_offers_is_active_idx").on(table.isActive),
  };
});
...
export const passportSocialShares = pgTable("passport_social_shares", {
  ...
  return {
    userIdIdx: index("passport_social_shares_user_id_idx").on(table.userId),
    sharedAtIdx: index("passport_social_shares_shared_at_idx").on(table.sharedAt),
  };
});
...
export const passportQrCheckins = pgTable("passport_qr_checkins", {
  ...
  return {
    ...
    eventIdIdx: index("passport_qr_checkins_event_id_idx").on(table.eventId),
  };
});
...
export const insertPassportUserAchievementSchema = createInsertSchema(passportUserAchievements)
  ...
  .extend({
    progressState: z.record(z.string(), z.any()).default({}),
  });
...
export const insertPassportRedemptionOfferSchema = createInsertSchema(passportRedemptionOffers)
  ...
  .extend({
    ...
    tierRequirement: z.enum(['BRONZE', 'SILVER', 'GOLD', 'ELITE']).optional(),
    fulfillmentMetadata: z.record(z.string(), z.any()).default({}),
  });
...
export const insertPromoterProfileSchema = createInsertSchema(promoterProfiles)
  ...
  .extend({
    ...
    billingAddress: z.record(z.string(), z.any()).default({}),
  });
