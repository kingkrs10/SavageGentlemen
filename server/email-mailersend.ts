...
export const sendEmail = async (options: EmailOptions): Promise<boolean> => {
  try {
    ...
    console.log(`[MAILERSEND] Attempting to send email to: ${to}, subject: ${subject}`);
    console.log(`[MAILERSEND] From: ${from}`);
    console.log(`[MAILERSEND] SMTP configured: ${process.env.MAILERSEND_SMTP_USERNAME ? 'Yes' : 'No'}`);
    
    // Verify transporter connection
    await transporter.verify();
    console.log('[MAILERSEND] SMTP connection verified successfully');
    ...
    console.log(`[MAILERSEND] Message prepared for sending`);
    
    const result = await transporter.sendMail(mailOptions);
    console.log(`[MAILERSEND] Email sent successfully:`, result.messageId);
    return true;
    ...
    if (error && typeof error === 'object' && 'response' in error) {
      console.error('[MAILERSEND] SMTP Error details:', error.response);
    }
...
/**
 * Send a ticket confirmation email with QR code using MailerSend
 */
export const sendTicketEmail = async (
  ...
  try {
    console.log(`[MAILERSEND] Generating QR code for ticket: ${ticketInfo.ticketId}`);
...
Questions? Contact us at: info@savgent.com
    ...
    if (result) {
      console.log(`[MAILERSEND] Ticket email sent successfully to ${recipientEmail}`);
    } else {
      console.error(`[MAILERSEND] Failed to send ticket email to ${recipientEmail}`);
    }
    
    return result;
    
  } catch (error) {
    console.error('[MAILERSEND] Error in sendTicketEmail:', error);
    return false;
  }
};
...
export const testEmailConnection = async (): Promise<boolean> => {
  try {
    console.log('[MAILERSEND] Testing email connection...');
    await transporter.verify();
    console.log('[MAILERSEND] Connection test successful');
    return true;
  } catch (error) {
